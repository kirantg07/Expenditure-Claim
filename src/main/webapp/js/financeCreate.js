...
hoursIndian = hours % 12;
hoursIndian = hoursIndian ? hoursIndian : 12; // Handle midnight
				...
				fetch('createAction', {
					...
					.then(data => {
						if (data.success) {
							...
							setTimeout(() => {
								window.location.href = 'financeCreate.jsp';
							}, 4000);
						} else {
...
function formatCurrency(amount1) {
	...
	let value = amount1;
	console.log("The value got from the input varible is " + value);
