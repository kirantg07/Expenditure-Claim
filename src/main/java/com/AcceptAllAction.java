...
@WebServlet("/AcceptAllAction")
public class AcceptAllAction extends HttpServlet {
	    ...
	    for (Object entry : newDataArray) {
	        ...
	        for (Object key : info.keySet()) {
	            ...
	            for (Object item : expenseItems) {
	                ...
	                if (expenseItem.get("id").toString().equals(id) && expenseItem.get("name").toString().equals(name)) {
...
//            if (key.equals(EC)) {
                ...
                // Sort expenseItems based on reclaimedDate if not empty, otherwise use claimedDate
...
//            }
	...
	private void writeUsersToJsonFile(JSONObject info) { Path path =
	  Paths.get(JSON_FILE_PATH);
	  
	  try (PrintWriter writer = new PrintWriter(path.toFile())) {
	  writer.print(info.toJSONString()); } catch (IOException e) {
	  e.printStackTrace(); } }
